...
public class ApiHandler
{
    ...
    public async Task<APIGatewayProxyResponse> HandleRequest(APIGatewayProxyRequest eventRequest, ILambdaContext context)
    {
        ...
        {
            return FormatResponseWithCors(400, new {message = $"Invalid request. Resource or HTTP Method is missing. Event request {eventRequest}. Context {context}"});
        }
    ...
    {
        return FormatResponseWithCors(400, new { message = $"Unsupported path or method: {path} {method}" });
    }

    private APIGatewayProxyResponse FormatResponseWithCors(int code, object response)
    {
    ...
    private async Task<APIGatewayProxyResponse> Signup(APIGatewayProxyRequest request)
    {
        ...
        {
            return FormatResponseWithCors(400, new { message = $"Missing required parameters: {string.Join(", ", missingParams)}" });
        }
        ...
        try
        {
            ...
            return FormatResponseWithCors(200, new { message = $"User {email} signed up successfully" });
        }
        catch (Exception ex)
        {
            return FormatResponseWithCors(400, new { message = ex.Message });
        }
    }
    ...
    private async Task<APIGatewayProxyResponse> Signin(APIGatewayProxyRequest request)
    {
        ...
        try
        {
            ...
            return FormatResponseWithCors(200, new { accessToken = token });
        }
        catch (Exception ex)
        {
            return FormatResponseWithCors(400, new { message = ex.Message + $" Request {request}" });
        }
    }
    ...
    private async Task<APIGatewayProxyResponse> GetTables(APIGatewayProxyRequest request)
    {
        ...
        return FormatResponseWithCors(200, new { tables });
    }
    ...
    private async Task<APIGatewayProxyResponse> AddTable(APIGatewayProxyRequest request)
    {
        try
        {
            ...
            catch (Exception ex)
            {
                ...
                return FormatResponseWithCors(400, new { message = "Invalid request body format." });
            }
            ...
            catch (Exception ex)
            {
                ...
                return FormatResponseWithCors(400, new { message = "Invalid field values." });
            }
            ...
            {
                return FormatResponseWithCors(500, new { message = "Error saving data to the database.", error = ex.Message });
            }

            var response = new { id = id };
            return FormatResponseWithCors(200, response);
        }
        catch (Exception ex)
        {
            return FormatResponseWithCors(500, new { message = "Other error occurred.", error = ex.Message });
        }
    }
    ...
    private async Task<APIGatewayProxyResponse> GetTableById(APIGatewayProxyRequest request)
    {
        ...
        {
            return FormatResponseWithCors(400, new { message = "Invalid or missing tableId" });
        }

        var table = await _tablesTable.GetItemAsync(tableId.ToString());
        if (table == null)
        {
            return FormatResponseWithCors(404, new { message = $"Table with id {tableId} not found" });
        }
    ...
    private async Task<APIGatewayProxyResponse> GetReservations(APIGatewayProxyRequest request)
    {
        ...
        return FormatResponseWithCors(200, new { reservations });
    }
    ...
    private async Task<APIGatewayProxyResponse> AddReservation(APIGatewayProxyRequest request)
    {
        try
        {
            ...
            try
            {
                ...
                {
                    return FormatResponseWithCors(400, new { message = $"Table with number {tableNumber} does not exist." });
                }
            }
            catch (Exception ex)
            {
                return FormatResponseWithCors(400, new { message = "Error checking table existence.", error = ex.Message });
            }
            ...
            foreach (var item in scanResult2)
            {
                ...
                {
                    return FormatResponseWithCors(400, new { message = "The reservation overlaps with an existing reservation for this table. slotTimeStart: " + slotTimeStart + ", slotTimeEnd: " + slotTimeEnd + ", existingSlotStart: " + existingSlotStart + ", existingSlotEnd " + existingSlotEnd });
                }
            }
            //////////////////////////////////
            ...
            {
                return FormatResponseWithCors(400, new { message = "Error saving reservation to the database.", error = ex.Message });
            }
        ...
        {
            return FormatResponseWithCors(400, new { message = "Other error occurred.", error = ex.Message });
        }
    }
